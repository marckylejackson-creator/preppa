...
type SwapTarget = { day: string; mealName: string } | null;
...
export function MealPlanView({ isGuest, onGuestAction }: Props) {
  ...
  const handleSwapClick = (day: string, mealName: string) => {
    if (isGuest && onGuestAction) { onGuestAction(); return; }
    setSwapTarget({ day, mealName });
  };

  const daysOrder = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"];
  const currentMealIds = plan?.meals.map((m: any) => m.mealId) ?? [];
    ...
    <>
              ...
              {daysOrder.map((day, idx) => {
                ...
                return (
                  <motion.div
                    ...
                    key={day}
                    className="flex items-center gap-3 p-3 rounded-2xl bg-secondary/30 border border-border/50 hover-elevate group"
                  >
                    <div className="w-24 shrink-0 font-display font-semibold text-sm text-muted-foreground">
                      {day}
                    </div>
                    <div className="flex-1 bg-card px-4 py-2.5 rounded-xl border border-border/50 premium-shadow min-w-0">
                    ...
                    <button
                      onClick={() => handleSwapClick(day, dayMeal.meal.name)}
      ...
      <AnimatePresence>
        {swapTarget && (
          <SwapModal
            day={swapTarget.day}
            currentMealName={swapTarget.mealName}
            currentPlanMealIds={currentMealIds}
