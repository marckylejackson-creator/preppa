...
import { Heart, Search, CalendarPlus, Trash2, X, Check, ChevronDown, Loader2 } from "lucide-react";
...
export function FavoritesManager() {
  ...
  // Map day -> current meal name
  const dayMealMap = useMemo(() => {
    if (!plan?.meals) return {} as Record<string, string>;
    const map: Record<string, string> = {};
    for (const entry of plan.meals) {
      map[entry.dayOfWeek] = entry.meal?.name ?? "Meal";
    }
    return map;
  }, [plan]);

  const removeFavMutation = useMutation({
    mutationFn: (updatedFavorites: string[]) =>
      apiRequest("PATCH", "/api/profile/favorites", { favorites: updatedFavorites }).then(r => r.json()),
    onSuccess: () => {
  ...
  const confirmDelete = (meal: string) => {
    ...
    removeFavMutation.mutate(updated);
  };

  const confirmAddToPlan = () => {
    if (!addingMeal || !selectedDay) return;
    addToPlanMutation.mutate({ mealName: addingMeal, day: selectedDay });
  };
    ...
    <div className="bg-card rounded-3xl p-6 premium-shadow border border-border/40">
      ...
      {profileLoading ? (
        ...
        <div className="min-h-[160px] flex flex-col items-center justify-center text-center">
          ...
          <p className="text-sm text-muted-foreground">No favorites saved.<br />Add some in your preferences.</p>
        </div>
      ) : filtered.length === 0 ? (
                      ...
                      >
                        <div className="px-4 pb-4 pt-1 border-t border-border/40 bg-secondary/20">
                            ...
                            {DAYS.map(day => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  {current && (
                                    <span className={`mt-0.5 font-normal truncate max-w-[56px] ${isSelected ? "text-primary-foreground/70" : "text-muted-foreground"}`}>
                                      {current.split(" ")[0]}
                                    </span>
                                  )}
                                </button>
                          ...
                          <div className="flex gap-2">
                            <button
                              onClick={() => { setAddingMeal(null); setSelectedDay(null); }}
                    ...
                    {deletingMeal === meal && (
                      ...
                      >
                        <div className="px-4 pb-4 pt-1 border-t border-destructive/20 bg-destructive/5">
                          ...
                          <div className="flex gap-2">
                            <button
                              onClick={() => { setDeletingMeal(null); setDeleteReason(null); }}
                              className="flex-1 py-2 text-xs font-semibold text-muted-foreground bg-secondary rounded-xl hover:bg-secondary/80 transition-colors"
                            >
                              Keep it
                            </button>
